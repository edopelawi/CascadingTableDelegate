...
class PropagatingTableDelegateDataSourceSpec: QuickSpec {
	...
	override func spec() {
		...
		beforeEach { 
			...
			propagatingTableDelegate = PropagatingTableDelegate(index: 20, childDelegates: childDelegates.map({ $0 as CascadingTableDelegate }))
		}
		...
		describe("tableView(_: numberOfRowsInSection:)") {
			...
			context("in .Section propagation mode", { 
				...
				it("should return 0 when passed with invalid section number", closure: { 
					...
					numberOfRows = propagatingTableDelegate.tableView(expectedTableView, numberOfRowsInSection: invalidSectionIndex)
